...
Initial layouts and snapshots share the same serializable tree idea: groups contain panels or groups, panels contain tabs, and a root layout can also contain floating panels. A snapshot preserves the panel tree, tab order, active tabs, fullscreen panel, reset targets, size constraints, behavior booleans, floating bounds, popout window bounds, and z-order.
...
`size` belongs to each child item and controls that item’s allocation inside its parent group. `defaultSize` is optional and defines where adjacent panels return when the user double-clicks a divider. If you omit `defaultSize`, Tilery uses the item’s initial `size` as the reset target.
