...
| P1       | Resize handle states and hit targets    | `react-resizable-panels`, Allotment                                                              | Partial       | `data-resize-active`, `data-resize-at-min`, `data-resize-at-max`, `data-resize-disabled`, and `resizeHandleHitSize` exist. Decide whether hover/focus need attributes beyond CSS.                              |
| P1       | Resize disabling and locking            | `react-resizable-panels`, Allotment, rc-dock, Dockview, FlexLayout, React-Grid-Layout, Gridstack | Partial       | Global resize disable and layout-item resize, drag, and drop locks exist via `resizable`, `draggable`, `droppable`, and `locked`. Add tab-level locks only if consumer-owned tab data is insufficient.         |
